...
// Daftar level/kelas di Rawacana
const LEVELS = [
  'Pra PAUD',
  'PAUD',
  'Paket A1',
...
export default function TpqRawacana() {
  // Get TPQ colors
  ...
  // Filter students untuk Rawacana & level terpilih
  const students = mockStudents.filter(
    s => s.tpqGroup === 'Rawacana' && s.isActive && s.level === selectedLevel
  );
  const totalSantri = mockStudents.filter(s => s.tpqGroup === 'Rawacana' && s.isActive).length;
  ...
  const attendanceForLevel = attendance.filter(a => {
    ...
    return student && student.tpqGroup === 'Rawacana' && student.level === selectedLevel;
  });
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className={`rounded-2xl p-6 border mb-4 ${colors.gradient} ${colors.border}`}>
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">
              TPQ <span className={colors.titleColor}>Rawacana</span>
            </h1>
            <p className="text-gray-600 text-lg">
              بِسْمِ اللّهِ الرَّحْمـَنِ الرَّحِيمِ
            </p>
            <p className="text-gray-500 text-sm mt-1">
              Selamat datang di halaman statistik dan progress TPQ Rawacana
            </p>
          </div>
      ...
      <Card className="mt-8 border-0 shadow-lg">
        <CardHeader>
          <CardTitle className="text-blue-800 font-bold">Informasi TPQ Rawacana</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-2 text-blue-900 text-base">
            <p><span className="font-semibold">Alamat:</span> Rawacana, Desa Jati</p>
      ...
      <Card className="mt-4 border-0 shadow-lg">
        <CardHeader>
          <CardTitle className="text-blue-800 font-bold">Daftar Generus {selectedLevel} - TPQ Rawacana</CardTitle>
        </CardHeader>
        <CardContent>
